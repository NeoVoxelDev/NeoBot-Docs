<!-- .vitepress/theme/components/VPCommunityScriptsEN.vue -->
...
const fetchScripts = async () => {
  try {
    ...
    error.value = 'Failed to load scripts. Please try again later.'
  } finally {
  ...
  <div class="vp-scripts-container">
    <div class="container">
      <div class="search-container">
        <input v-model="searchQuery" type="text" placeholder="Search script name, ID, author, description or type..."
          class="search-input" />
      </div>
      ...
      <div v-else-if="error" class="error">
        <p>{{ error }}</p>
        <button @click="fetchScripts" class="retry-button">Retry</button>
      </div>

      <div v-else-if="filteredScripts.length === 0" class="empty">
        <p>{{ searchQuery ? 'No matching scripts found' : 'No scripts available' }}</p>
      </div>
