...
const updateScrollProgress = () => {
  // 如果使用自定义平滑滚动，从实例获取当前滚动位置
  let currentScrollY;
  if (window.smoothScroll) {
    // 从平滑滚动实例获取当前滚动位置
    currentScrollY = window.smoothScroll.currentScrollPosition || window.scrollY;
  } else {
  ...
  scrollProgress.value = totalScroll > 0 ? Math.min(currentScrollY / totalScroll, 1) : 0;
};
...
onMounted(() => {
  ...
  // 如果使用平滑滚动，添加额外的监听器
  if (window.smoothScroll) {
    // 使用 setInterval 来定期检查滚动位置
    const checkInterval = setInterval(() => {
      handleScroll();
    }, 50); // 每50ms检查一次

    // 保存interval ID以便清理
    window.backToTopInterval = checkInterval;
  }
...
onBeforeUnmount(() => {
  ...
  // 清理interval
  const intervalId = window.backToTopInterval;
  if (intervalId) {
    clearInterval(intervalId);
    window.backToTopInterval = null;
  }
});
</script>
